...
class BeamSearchNode(object):
    ...
    def eval(self, beta, w):
        ...
        seq_p = self.log_p / float(self.len - 1 + 1e-6)
        additional_p = (w['coherence'] * self.coherence + \
                       w['generality'] * self.generality + \
                       w['simplicity'] * self.simplicity)

        return (alpha * seq_p) + (beta * additional_p)


decoder = DecoderRNN()


def beam_decode_for_condition(decoder, target_tensor, decoder_hiddens, encoder_outputs=None):
    '''
    ...
    # Iterate over instances within a batch
