...
def train(attr_input, topic_input, topic_output, mask, encoder, topic_decoder, encoder_optimizer, topic_decoder_optimizer):
    ...
    loss += mask_loss
    print('mask_loss :', mask_loss.data)
...
def trainIters(corpus, learning_rate, lr_decay_epoch, lr_decay_ratio, batch_size, n_layers, hidden_size, embed_size, 
    ...
    print('u_embedding: ', u_embedding)
    print('from_pretrained: ', torch.eye(max_rating), torch.zeros(max_rating, context_emb_size-max_rating))
    cat_mat = torch.cat((torch.eye(max_rating), torch.zeros(max_rating, context_emb_size-max_rating)), dim=1)
    print('torch.cat: ', cat_mat.shape)
    ...
    attn_model = 'dot'
    topic_decoder = AspectAttnDecoder(t_emb, embed_size, hidden_size, context_emb_size, vocab.n_topics, n_layers)
